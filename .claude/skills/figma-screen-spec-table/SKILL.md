...
## 61. 엑셀 UI 정의서를 기획서 표에 「중복은 두고 신규만」 반영할 때

- 시트를 셀 좌표째 덤프해(openpyxl) 항목 ①~⑤ 를 표 행에 1:1 로 대응시킨다. 같은 뜻의 줄은 건드리지 않고, 없는 정책만 **기존 셀 끝에 줄 추가**(행 삭제·재번호 없음 → 마커도 그대로).
- 예시 표기는 정의서 원문 그대로(`26.09.01ㅣ13:00 ~ 15:00`, `4회차ㅣ기본제공(월) 3회`) 넣고, 기존 예시(`2026년09월01일`)와 형식이 다르면 보고에 적는다.
- 멱등성: 셀에 키워드(「운영자」「배정번호」「취소가능」)가 이미 있으면 건너뛰게 해 두면 타임아웃 뒤 재실행이 안전하다.
- 5셀 스왑도 한 호출에서 60초를 넘길 수 있다. 타임아웃 뒤엔 셀별 font/textStyleId 를 다시 읽어 적용 여부를 확인한다(이번엔 모두 적용됨).
- 정의서와 목업이 어긋나는 부분(편의 카드 버튼이 「신청취소」인데 정책은 주문취소/예매취소)은 목업을 고치지 않고 보고한다.

## 62. 목업 버튼 라벨을 정책에 맞출 때 — 마스터를 고치면 인스턴스 오버라이드를 되돌려 준다

- 편의 카드 버튼 「신청취소」→ 카페 「주문취소」, 영화 「예매취소」. 텍스트는 Pretendard Bold 바인딩(`S:d3255e74…`) → `swap/14-20--6b`(로컬 스타일에서 이름으로 찾음) 로 스왑 편집.
- 카드 1이 COMPONENT 마스터이고 카드 2가 그 인스턴스면, 마스터를 「주문취소」로 바꾸면 카드 2(H 툴스)도 따라 바뀐다. **카드 2 텍스트를 「신청취소」로 다시 써서 오버라이드**로 고정한다.
- 분리 복제된 장의 COMPONENT 는 원본과 별개 마스터라 다른 장에는 영향이 없다.

## 63. ★ 날짜·횟수 표기는 덱의 기존 형식에 맞춘다 / 마커는 편집 후 전부 재검증한다 (사용자 지적)

- 정의서 예시를 옮길 때 `26.09.01ㅣ13:00` 처럼 **원문 형식을 그대로 넣지 않는다**. 덱은 `2026년09월01일 / 13:00` 형식이다. 사용자가 직접 고쳐야 했다.
  단, 사용자가 「엑셀과 동일하게」라고 지정한 항목(횟수 `4회차ㅣ기본제공(월) 3회`)은 원문 그대로.
- 표 셀을 고칠 때 사용자가 그 사이 편집했을 수 있다 → **현재 characters 를 읽어 줄 단위로 필터·추가**하고, 이전에 내가 넣은 문장을 통째로 다시 쓰지 않는다.
- 편집을 마치면 3장 모두 마커 중심 − 대상 중심 diff 를 표로 찍는다(스펙: 라벨→정규식/카드). diff 0 이 아닌 것만 고친다.
...
## 65. 목업 예시값과 표 예시가 어긋나면 표를 목업 형식에 맞춘다 (영화 카드 상영시간)

- 정책이 「상영일＋시간(시작/종료)」이면 목업 캡션도 `2026년09월01일 / 13:00 ~ 15:30` 로 채운다(덱 날짜 형식, 구분자 `/`). 제목 옆의 시간은 사용자가 지웠으므로 표 2행의 「영화제목 | 13:00」도 「영화제목 (예시 : …)」로 맞춘다.
- 사용자가 목업을 먼저 고쳐 놓는 경우가 많다. 텍스트를 바꾸기 전 카드의 TEXT 목록을 다시 읽어 **현재 상태 기준**으로 표만 맞춘다.
- 60초 타임아웃이 나도 한 노드 편집은 대개 적용되어 있다 — 다시 읽어 확인만 한다.

## 66. 정책별 카드 세트를 두 디자인 파일에 만들 때 (신청현황 카드 9종)

- 디에이치 카드 = 로컬 컴포넌트 셋 `dash-card`(55606:146487) 의 `Property 1=number`(12330:232922). `createInstance()` 후 텍스트는 **기본 characters 로 찾는다**
  (관리비→카테고리, 김명표→"" 사용자명 제거, 조식 2인→커뮤니티명, 미디어룸→배정번호(ffb03a 강조), 이용기간→라벨, 2026년 8월→값, 신청취소→버튼).
  ★ 같은 기본값을 두 번 찾으면 첫 번째 것을 다시 잡는다(라벨을 「이용기간」으로 쓴 뒤 두 번째 「이용기간」을 지우려다 라벨이 사라졌다) — 값을 먼저 바꾸고 나서 다음을 찾거나 id 접미사로 찾는다.
  숨겨진 두 번째 Label Text(55606:147240)는 인스턴스 `findAll` 에 안 잡힌다 → 횟수는 값 한 칸에 `4회차ㅣ기본제공(월) 3회` 로 쓴다.
  배정번호 없는 카드는 구분선 `Rectangle 40177` opacity 0. 버튼 없는 카드는 `detachInstance()` 후 버튼 인스턴스 `remove()`.
  폰트는 `Noto Sans` Regular/Bold(로드됨). 섹션 배경은 앱 배경색(2a1f1c)으로 칠해야 10% 흰색 카드가 보인다.
- 마이힐스는 카드 컴포넌트가 원격(`list box/343/community/infomation01`)이라 인스턴스 `clone()`→`detachInstance()` 로 프레임을 만든 뒤 디에이치 구조로 재배치한다
  (뱃지 16,16 · 제목 Bold20 y48 · 구분선 `|` + 배정번호 971934 · date01 텍스트 y88 「라벨  값」 · 버튼 98×32 우하단 16px). `resize` 는 SCALE 제약으로 자식이 늘어나므로
  이후 `textAutoResize="WIDTH_AND_HEIGHT"` 와 버튼 `resize(98,32)` 를 다시 건다. 「자동등록 이용중」 같은 상태 텍스트는 remove.
- 카드 순서(둘 다 동일): 01 이용기간＋번호/신청취소 · 02 이용일/예약취소 · 03 이용일＋시간(시작) · 04 이용일＋시간(시작/종료) · 05 횟수제(월) · 06 강좌 횟수제(일) · 07 수강기간·버튼 미표출 · 08 카페 주문번호/주문취소 · 09 영화 상영일＋시간/예매취소.
  날짜는 디자인 파일 상세와 같은 `2026.09.01`, 시간은 두 칸 띄고 `13:00 ~ 15:00`. 횟수는 엑셀 원문.
- 플러그인 실행이 중간에 에러 나면 그 호출의 변경은 **전부 남지 않는다**(섹션까지 없어짐) — 한 호출 안에서 완결되게 쓰고, 다음 호출에서 존재 여부를 먼저 확인한다.

## 67. ★ 사용자가 링크만 보내면 「여기 있는 디자인이 기준」이라는 뜻 — 먼저 그 섹션을 읽고 내가 만든 것을 거기에 맞춘다

- 마이힐스 신청현황 카드의 진짜 디자인은 섹션 「2026.09.15」(58752:321190): 로컬 컴포넌트 `list box/343/community/infomation03`(58752:321952) + 화면 11장(시설 4 · 강좌 4 · 편의 3).
  옛 샘플(H-helper_sample) 카드로 만든 내 정책별 카드는 이 컴포넌트 인스턴스로 다시 만들었다. 페이지 전수 검색이 「신청현황」 헤더를 못 찾았던 이유는 타임박스가 아니라 정규식(`^신청현황$`)이 다른 텍스트 구성을 놓친 것 — 링크를 받으면 그냥 그 노드를 읽는다.
- 컴포넌트 텍스트 접미사: 뱃지 `56863:613905` · 제목 `58752:320775` · 배정번호 `58752:320777`(구분선 `58752:320776`) · date01 두 칸 `138:93425`(회색) / `138:93427`(진한색) · 버튼 `13:1028`.
  사용자명 `58752:321184` 는 **마스터에서 remove** → 모든 인스턴스에서 사라진다(오버라이드 「김명표」 포함).
- 정책 반영 규칙(디자인 파일): 사용자명 삭제 / 이용일·시간 카드 = 예약취소, 월권·횟수 = 신청취소, 카페 = 주문취소, 영화 = 예매취소 / 횟수 = `4회차` + `기본제공(월) 3회` 두 칸 / 영화는 제목 옆 시간을 비우고 상영일자 칸에 `날짜  13:00 ~ 15:30`.
  날짜는 그 파일 화면의 형식(마이힐스 화면은 `2026년 09월 01일`, 상세는 `2026.09.01`)을 따르고 바꾸지 않는다. 「09월 31일」 같은 사용자의 예시값은 두고 보고만 한다.
- 디에이치 목록 화면의 dash-card 는 마스터를 건드리지 않고 인스턴스마다 이름 텍스트(`372:42575`)를 "" 로 — 마스터의 btn-view-all 은 다른 곳에서도 쓰일 수 있다.
- (§67 보충) 마이힐스 카드의 date01 첫 칸에 「이용일자」 같은 라벨을 넣지 않는다 — 날짜만 첫 칸에, 둘째 칸은 시간이나 비움(사용자 지시 「이용일자 텍스트 적용하지 않음」).
...
## 71. 「기획서 정책에 맞게 카드 내용 수정」— 케이스 카드 13종의 기준 문안

- 칩 = 운영자 카테고리명(독서실·필라테스·게스트하우스·문화강좌·사우나·영화·카페), 제목 = 커뮤니티명(+ `| N번` 배정번호), 부가정보 = 케이스별 형식
  (이용일＋시간 `2026년 09월 25일  13:00 ~ 15:00` · 기간 `2026년 09월 01일 ~ 2026년 09월 30일` · 횟수 `4회차ㅣ기본제공(월) 3회` · `주문번호  1264567890` · 상영일＋시간), 버튼 = 유형별(예약/신청/주문/예매취소, 취소 불가 케이스는 없음).
- 빈 상태(케이스 01)는 기획서 2행 「신청 · 예약 건이 없을 때 카드 영역 표시하지 않음」에 따라 `section-title`(신청현황 헤더)과 빈 카드 `section` 을 지운다. `wrap` 이 VERTICAL 오토레이아웃이라 아래 내용이 자동으로 올라온다(앱 캡처의 「예정된 신청 내역이 없습니다」 카드와 다름 → 보고).
- 인스턴스 카드에 버튼을 넣어야 하면 `detachInstance()` 후 `Date Text` 의 `parent.parent`(Frame 2612343)에 버튼 clone. 이름으로 찾는 `Frame 2612343` 은 detach 직후 없을 수 있다.
- 플러그인 호출은 에러 시 전체 롤백 → 케이스 01 처럼 구조가 다른 화면은 별도 호출로 분리한다.

## 72. 사용자 규칙 추가 — 케이스 화면 제목·문안·날짜

- 전달받은 PPT 의 케이스 **제목을 그대로** 화면 이름과 화면 위 라벨(`case-label <id>`, Noto Sans 16)로 쓴다. 줄여 쓰지 않는다.
- 카드의 커뮤니티명·카테고리 칩·날짜도 **PPT 캡처 값 그대로**(방배 / A블록 악기연주실1 · 커뮤니티센터 / 개인독서실(남) · 힐스카페 …). 내가 만든 예시(독서실 (남) | 1000번)로 바꾸지 않는다.
- 날짜는 항상 **오늘 이후의 최신 날짜**(오늘 2026-09-22 → 09월 24일~10월). 지난 달·이전 날짜 예시를 쓰지 않는다.
- 빈 상태는 디자인(앱)대로 「예정된 신청 내역이 없습니다.」 안내 카드를 두고, 기획서 쪽 문장을 디자인에 맞춰 고친다(카드 3장 표 2행). 기획서와 디자인이 다르면 디자인을 기준으로 기획서를 수정하라는 지시였다.
- 빈 카드 복원: 케이스 02 의 `section-title` 과 `section` 을 clone 해 `wrap` 의 contents 앞에 insert, 카드는 034 - 2 의 빈 dash-card clone.

## 73. 상세 정의서(xlsx 예약현황_상세)를 유형별 6장으로 분리해 만들 때

- 페이지 = 카테고리(시설 · 강좌) × 유형(신청 · 예약 · 이용). 각 장 폰 2대 = 상태 「확정」 / 「취소」(취소 장은 취소일시 행 추가, 자동등록·알림 블록과 취소 버튼 제거).
- 상세 목업(예약확인_시설 프레임)은 텍스트가 **Noto Sans KR Medium 미바인딩**이라 characters 직접 대입 가능. 행 = `Frame 607` 안의 GROUP(라벨 + 값) → 첫 그룹을 clone 해 템플릿으로 쓰고 전부 지운 뒤 필요한 수만큼 clone, 값 텍스트는 `x = 282 − width` 로 우측 정렬.
  행 간격 36(22+14). 폰 1 콘텐츠 하단(`Frame 632`)이 버튼(y 646)을 넘지 않게 8행까지.
- 표 셀은 Pretendard 바인딩(S:9b54efdf…) → `swap/14-24--6` 스왑 편집. 1줄 행(`Text`)을 템플릿으로 clone 해 0~10 행을 새로 만들고 Switch·Button 행은 재사용(번호만 11·12). 행 높이 1줄 45/49 · 2줄 73 · 3줄 97 · 5줄 145 — 표 바닥 ≤ 1018 이 되도록 설명·앱바·옵션 행은 1~2줄로 줄인다.
- 마커 `Number sign/left` 텍스트는 **Pretendard Bold** → `swap/badge 12 (fix)` 로 스왑 후 원래 스타일(S:6199c25c…)로 복귀. 위치: 1 앱바 316 · 2 썸네일 370 · 3 제목 404 · 행 i → 460+36i · 자동등록 → f607 높이+21+1+21+ (262+199) −2 · 버튼 917. 템플릿 마커를 먼저 clone 한 뒤 원본을 지운다(먼저 지우면 롤백).
- 제목은 이 장들에서 Noto Sans KR Bold 51.2 스타일(S:cc6a1702…). 「홈_ 커뮤니티_」를 빼 표(x 1240)와 겹치지 않게.
- 한 호출에 clone + 폰 2대 + 표 + 마커를 다 넣으면 60초 초과 → clone / 제목·폰 / 표 / 마커 4개 호출로 나눈다. clone 위치는 `insertChild` 뒤 `indexOf` 로 확인하고 마지막에 한 번에 재정렬 + 구 페이지 삭제.
- 슬라이드 이름은 사용자가 번호("574")로 자동 관리한다 — 이름을 의미 있게 붙여도 곧 덮어씌워지므로 제목 텍스트로 식별한다.
- (§72 보충) 카페 카드는 이후 「정책에 맞춰」로 되돌렸다: 칩 업장명(힐스카페) · 상품명 · `주문번호  1264567890` · 주문취소 버튼(취소가능 기간). 영화는 디자인 기준(버튼 없음) 유지. 기획서 편의 (3/3) 취소 행도 두 줄로 나눠 같은 내용을 적었다 — 카드별로 기준이 다를 수 있으니 지시가 바뀌면 카드 하나만 바꾸고 기획서 줄도 그 카드 줄만 고친다.

## 74. 페이지 설명은 그 페이지 내용만 — 다른 유형·다른 탭 언급을 지운다 (사용자 지시)

- 상세 6장 0행의 「카페 스마트오더 · 영화 예매는 별도 기획 적용」, 강좌 장 7행의 「(시설 이용기간과 동일)」 같은 교차 언급 삭제.
- 카드 3장의 「카드 [Tap]」 행은 그 탭 것만(시설 장 = 시설 상세 이동, 강좌 장 = 강좌 상세 이동, 편의 장 = H 툴스 · 카페 · 영화), 편의 장 취소 행 첫 줄은 「주문취소」로.
- 정규식 치환은 멱등하게 쓰고, 타임아웃 뒤 재실행 시 swap 스타일에 남은 셀은 같은 표 1행의 textStyleId 로 복구한다.
- (§73 보충) 요금 상세는 목업에서 「요금(상세)」 라벨 행 + 값 칸에 `항목1  5,000원`, 다음 행 라벨 비우고 `항목2  6,000원`, 그다음 「요금(합계)」 로 표현한다(항목1/항목2 를 라벨로 쓰지 않는다). 12개 폰을 한 호출에 고치면 60초를 넘겨도 대개 적용되어 있으니 그룹 이름(`요금(상세) 행`)으로 멱등 확인.
- (§73 보충) 사용자가 목업을 먼저 고친 장(H 툴스)은 「화면에 맞춰 기획 수정」: 목업 행 순서대로 표 4행을 다시 쓰고, 그 장과 무관한 Info 행(카페·영화)은 지운다. 「실행」 지시로 요금(합계) 값(6,000원)과 결제 수단 행을 목업에 추가 → 아래 블록이 36px 내려가므로 마커 5(알림받기)를 텍스트 중심으로 재정렬.
