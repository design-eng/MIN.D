...
// ══════════════════ 11. 인사 운영 매뉴얼 ══════════════════
// 문서는 다 만들어졌다. 이 매뉴얼은 그 문서들을 언제 누가 돌리는지를 정한다.
const op = [];
op.push(
  P_(CO.name, {size:19, color:GRAY, after:60}),
  new Paragraph({ spacing:{after:60}, children:[T('인사 운영 매뉴얼', {size:32, bold:true})] }),
  new Paragraph({ spacing:{after:300}, border:{bottom:{...med, space:8}}, children:[T('')] }),
  gap(60),
  hang('취업규칙과 각 지침이 정한 것을 실제로 언제 누가 하는지 정리한 문서이다. '
    + '제도를 새로 만드는 것이 아니라, 이미 정해진 것이 빠짐없이 돌아가게 하는 데 목적이 있다.'),
  why('대표가 상시 사무실에 있지 아니한 상황을 전제로 만들었다. 기억에 의존하지 아니하고 '
    + '정해진 시점에 정해진 화면을 열어 확인하는 방식으로 구성하였다.'));

op.push(h2('1. 매일'),
  table(['무엇','누가','어디에'], [
    ['출근·퇴근 시각 기록','사원 본인','관리대장 — 근태기록'],
    ['휴게 시작·종료 기록','사원 본인','관리대장 — 근태기록'],
    ['그날 작업한 프로젝트 기록','사원 본인','관리대장 — 근태기록 M열'],
  ], [3000,1700,3500]),
  gap(120),
  why('프로젝트 칸은 견적의 근거가 되고, 대표가 자리에 없어도 무엇에 시간이 들어갔는지 '
    + '알 수 있게 한다. 출퇴근 시각보다 이 칸이 더 쓸모 있다.'));

op.push(h2('2. 매월'),
  rule('말일 — 급여 지급일'),
  ho('1. 임금 지급. 지급일이 휴일이면 그 전일에 지급한다. (취업규칙 제31조제1항·제2항)'),
  ho('2. 임금명세서 교부. 구성항목·계산방법·공제내역을 적는다. (제32조)'),
  ho('3. 복지비 영수증 마감. 사원이 제출한 내역을 확인한다. (복지카드 운영지침 제4조)'),
  rule('다음 달 초 — 전월 마감 (대표)'),
  ho('1. 관리대장 「월별집계」 시트를 연다. B3 칸의 조회 월을 전월 1일로 바꾼다.'),
  ho('2. 근무일수·총 근무시간·연장시간·지각·결근·연차사용을 확인한다.'),
  ho('3. 연장시간이 있으면 「보상휴가」 시트에서 발생·사용·잔여를 확인한다.'),
  ho('4. 전월에 발생한 연장·야간·휴일근로의 가산수당 또는 보상휴가를 이번 달 급여에 반영한다. (제31조제3항)'),
  ho('5. 복지비를 이번 달 급여와 함께 지급한다. (복지카드 운영지침 제4조제3호)'),
  gap(120),
  why('이 다섯 줄이 이 매뉴얼에서 가장 중요하다. 매일 기록이 아무리 잘 쌓여도 한 달에 한 번 '
    + '누가 열어보지 아니하면 아무 일도 일어나지 아니한다. 15분이면 끝난다.'));

op.push(h2('3. 분기'),
  table(['시점','무엇','근거'], [
    ['분기 초','연차 잔여와 소멸일 확인, 사용할 날짜를 사원과 정한다','제23조'],
    ['분기 초','보상휴가 잔여와 3개월 사용기한 도래분 확인','제21조제5항'],
    ['수시','주 12시간 연장근로 한도 초과 여부 확인','제21조제1항'],
  ], [1500,4400,2300]),
  gap(120),
  why('연차는 「쓰고 싶을 때 쓰라」고 하면 쓰지 아니한다. 분기마다 범위를 주고 사원이 '
    + '날짜를 고르게 하는 방식이라야 실제로 사용된다.'));

op.push(h2('4. 매년'),
  table(['시점','무엇','근거'], [
    ['6월 30일까지','하계휴가 희망 기간 제출받기','제24조제2항제1호'],
    ['연 1회','성희롱 예방·개인정보 보호·산업안전·장애인 인식개선 교육','제44조'],
    ['2년 1회','사무직 일반건강진단','제42조'],
    ['연 1회','복지비 금액과 항목 재검토. 변경 시 30일 전 통지','복지카드 운영지침 제6조'],
    ['연 1회','보상휴가제 서면합의 유효기간 확인','서면합의서 제9조'],
  ], [1900,4400,1900]));

op.push(h2('5. 사원별로 돌아오는 일정'),
  hang('연차와 연봉은 입사일을 기준으로 사람마다 다른 날에 돌아온다. 입사할 때 아래 표를 채워 둔다.'),
  gap(80),
  table(['무엇','언제','근거'], [
    ['수습 종료','입사일부터 3개월','제9조제1항'],
    ['연차 산정기간','입사일부터 매 1년','제22조제3항'],
    ['연차 소멸','입사 1주년 전날 (1년 미만분)','제23조제4항'],
    ['연봉 조정 면담','조정 적용일이 속한 달의 전달까지','제34조제2항'],
    ['연봉 조정 적용','입사 기념일이 속한 달의 다음 달 1일','제34조제1항'],
    ['퇴직급여 발생','계속근로 1년 이상','제40조'],
  ], [2200,4000,2000]),
  gap(160),
  rule('강성아 (입사 2026년 1월 12일)'),
  table(['시점','무엇'], [
    ['2026년 4월 11일','수습 종료 — 완료'],
    ['2026년 12월 중','연차 잔여 점검. 소멸이 한 달 앞이다'],
    ['2027년 1월 11일','연차 소멸. 미사용분은 수당으로 지급 (제23조제4항)'],
    ['2027년 1월 중','연봉 조정 면담 (제34조제2항)'],
    ['2027년 1월 12일','계속근로 1년 — 퇴직급여 발생 (제40조)'],
    ['2027년 2월 1일','조정 연봉 적용 (제34조제1항)'],
  ], [2400,5800]),
  gap(120),
  why('2027년 1월에 연차 소멸·연봉 면담·퇴직급여 발생이 겹친다. 2026년 12월에 미리 준비한다.'));

op.push(h2('6. 기한이 정해진 것'),
  hang('놓치면 되돌릴 수 없거나 법 위반이 되는 것들이다.'),
  gap(80),
  table(['무엇','기한','근거'], [
    ['임금 지급','당월 말일','제31조제1항'],
    ['변동급 정산','사유 발생한 달의 다음 달 임금 지급일','제31조제3항'],
    ['보상휴가 사용','발생한 달의 말일부터 3개월','제21조제5항'],
    ['미사용 보상휴가 수당','기간이 끝난 달의 다음 달 임금 지급일','제21조제5항'],
    ['연차 미사용 수당','산정기간 종료 후','제23조제4항'],
    ['퇴직 시 금품 청산','퇴직일부터 14일','근로기준법 제36조'],
    ['퇴직 시 보상휴가 정산','퇴직일부터 14일','서면합의서 제7조제3항'],
    ['해고 예고','30일 전','제38조'],
    ['근태·임금 서류 보존','3년','근로기준법 제42조'],
  ], [2900,3200,2100]),
  gap(120),
  why('근로기준법 제36조와 제42조는 상시 근로자 5명 미만 사업장에도 적용된다. '
    + '퇴직자의 근로계약서와 근태기록은 퇴직일부터 3년간 보존한다.'));

op.push(h2('7. 대표가 사무실에 없을 때'),
  ho('1. 근태는 사원이 스스로 기록한다. 대표의 확인은 월 마감 때 한 번이면 충분하다.'),
  ho('2. 진행 상황은 근태가 아니라 공유 드라이브의 산출물로 확인한다. 업무 프로세스의 파일명 규칙과 저장 위치 규칙이 이를 위한 것이다.'),
  ho('3. 작업 기간의 절반 시점에 1차 내부 컨펌을 받는다. 이것이 지켜지면 방향이 크게 어긋나는 일이 없다. (업무 프로세스)'),
  ho('4. 주 1회 짧게 이야기하는 자리를 정해 둔다. 이번 주 한 일, 다음 주 할 일, 막힌 것 세 가지면 된다.'),
  ho('5. 사원이 휴가로 자리를 비우는 날의 전화·메일 처리 방법을 미리 정해 둔다. 이것이 없으면 휴가를 쓰라고 해도 쓰지 못한다.'),
  gap(120),
  why('제2호가 핵심이다. 출퇴근 시각은 자리에 있었는지만 알려주고 무엇을 했는지는 알려주지 아니한다. '
    + '산출물이 훨씬 정확한 신호이며, 감시로 느껴지지도 아니한다.'));

op.push(h2('8. 문서가 있는 곳'),
  table(['문서','언제 쓰는가'], [
    ['취업규칙','근로조건에 관한 판단이 필요할 때. 게시하여 전 사원이 열람한다'],
    ['근무정책 안내서','사원이 규정을 쉽게 찾아볼 때'],
    ['근태연차 관리대장','매일 기록, 매월 마감'],
    ['보상휴가제 서면합의서','연장근로를 휴가로 갈음할 때의 근거'],
    ['복지카드 운영지침','복지비 사용 판단'],
    ['업무 프로세스','프로젝트 진행 방식'],
    ['연봉조정 운영기준·평가 서식','연 1회 연봉 조정 때'],
    ['근로계약서·정정합의서','개인별 서류. 인력관리 폴더에 보관'],
  ], [2700,5500]),
  gap(140),
  hang('개인정보가 담긴 서류는 회사 공유 드라이브의 인력관리 폴더에만 둔다. 개인 저장소에 두지 아니한다.'));

op.push(gap(300),
  P_('제정일        년        월        일', {align:AlignmentType.CENTER, after:130}),
  P_(CO.name, {align:AlignmentType.CENTER, bold:true, size:22, after:100}),
  P_('대표    ' + CO.ceo, {align:AlignmentType.CENTER, size:20}));

docs.push({ name:'마인드_인사운영_매뉴얼.docx', body:op });
