...
guide = [
    ...
    ('5인이 되면', '상시 5명 이상이 되는 순간 연차유급휴가·가산수당·연차사용촉진이 모두 법적 의무가 됩니다. '
                '취업규칙과 이 대장을 함께 점검하세요.'),
]
r = 4
for k, v in guide:
...
at.cell(row=nrow, column=1,
        value='· 「연장시간」은 1일 8시간 초과분입니다. 마인드는 5인 미만 사업장이므로 가산수당(50%) 지급의무는 없고, '
              '실제 근로시간에 대한 통상임금만 지급합니다. (취업규칙 제21조제2항)')
at.cell(row=nrow + 1, column=1,
        value='· 5명 이상이 되면 이 열의 시간에 통상임금의 50%를 가산해야 하고, 주 12시간 연장 한도도 적용됩니다.')
for i in range(2):
